...
`morphology-patterns.ts` is a DIFFERENT kind of derived file: it is regenerated at **authoring time** (not publish time) by `scripts/generate-morphology-patterns.ts` from the lean, hand-authored `morphology-roots.ts` (the deterministic affix engine + slug mint), then committed — like `learning-items.ts`/`grammar-patterns.ts` are canonical-but-regenerable. Edit `morphology-roots.ts` and re-run the script; never hand-edit `morphology-patterns.ts`. It is a lesson-stage INPUT, so — unlike the three runner-derived files above — it stays in the committed staging set and is read by the publish, not regenerated by the capability-stage runner.
