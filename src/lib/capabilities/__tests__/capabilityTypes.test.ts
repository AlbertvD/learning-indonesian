...
describe('deriveSkillTypeFromCapabilityType', () => {
  ...
  it('maps recognise_form_from_meaning_cap to recognition (Slice 1 mis-level fix)', () => {
    expect(deriveSkillTypeFromCapabilityType('recognise_form_from_meaning_cap')).toBe('recognition')
  })

  it('keeps meaning_recall on the meaning_recall skill', () => {
    expect(deriveSkillTypeFromCapabilityType('recall_meaning_from_text_cap')).toBe('meaning_recall')
  })
