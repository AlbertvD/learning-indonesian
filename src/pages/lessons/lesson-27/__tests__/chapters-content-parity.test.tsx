// Content-parity guard for the lesson-27 chapter conversion.
//
...
describe('lesson 27 chapters — content parity with content.json', () => {
  ...
  it('renders every paragraph of the house tour reading', () => {
  ...
  it('renders the formal letter (head, salutation, punch-list, sign-off)', () => {
    const c = content.sections[1].content as { paragraphs: string[] }
    const [head, salutation, body, signoff] = c.paragraphs
    expectPresent(head)
    expectPresent(salutation)
    const lines = body.split('\n')
    const intro = lines.filter((l) => !/^\d+\./.test(l.trim()))
    intro.forEach(expectPresent)
    lines
  ...
  it('renders the proverb (Indonesian + Dutch)', () => {
