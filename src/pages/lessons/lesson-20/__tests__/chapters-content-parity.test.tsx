// Content-parity guard for the lesson-20 chapter conversion.
//
 ...
 *  at inline-markup boundaries (e.g. <em>) — either would false-negative a
 *  plain substring check. Content equality is what we guard, not spacing.
 *
 *  Also drop dashes/en-dash, commas, colons, slashes, parens and arrows:
 *  GrammarExamples' transform mode splits a "base → derived" indonesian
 *  field on "→" and re-renders base/arrow/derived as three sibling spans
 *  (Page.tsx GrammarExamples, mode='transform') — a separator transform, not
 *  a content drop, but it means the raw needle and the rendered haystack can
 *  legitimately differ in surrounding whitespace/punctuation around the
 *  arrow. Stripping both sides identically keeps the check about content,
 *  not presentation (mirrors lesson-2's richer normalise()). */
function normalise(s: string): string {
  return s.replace(/[\s–—,:/>→()-]+/g, '')
}

describe('lesson 20 chapters — content parity with content.json', () => {
  ...
  it('renders every taste vocabulary item', () => {
  ...
  it('renders every paragraph of the Bali ferry reading passage', () => {
    const c = content.sections[3].content as { paragraphs: string[] }
    c.paragraphs.forEach(expectPresent)
  })

  it('renders every vocabulary item from the reading-passage glossary', () => {
  ...
  it('renders every grammar category (concept, allomorphs, semantic asides) with title, rules and examples', () => {
