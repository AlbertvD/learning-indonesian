...
5. **Minimum mechanism** — the design uses the fewest moving parts that meet the goal (CLAUDE.md "Minimum Mechanism"). For every table/column/function/generated-column/trigger/gate/enum value/abstraction, the spec states what breaks if omitted; cut anything whose only justification is a problem another part of the *same* design created (cut both). Prefer a pre-write validator or a module-load assertion over DB generated columns/triggers + a sync check unless a non-pipeline writer needs DB enforcement. Re-derive an `approved` spec against CLAUDE.md "Operating Context" — **rewritten 2026-07-02 at the commercialization flip: there ARE real users and learner data is precious.** Safety machinery that protects users or their data (parity tests for transport changes, gated learner-data migrations, honest failure copy, non-vacuous health checks) now EARNS its keep and passes this gate; what remains over-engineering is parity rollouts / coexistence layers for CONTENT tables (rebuild-friendly per Operating Context) and mechanism whose only justification is a problem the same design created. **Over-engineering fails this gate exactly as under-engineering does** — durable target-state ≠ maximal.
...
3b. **Reuse-and-simplify first — a PREMISE check, run BEFORE you validate the proposed mechanism.** Ask: does an existing module, session/feature **mode**, scope, flag, RPC, or `CONTEXT.md` definition already provide this capability (even in a more general form)? Grep/read for it. If the proposal builds a NEW mechanism — a parallel engine, a shared extraction, a new table/column/type, or a second definition of an existing concept — for something the platform already does, that is at minimum a **WARNING**: name the existing feature and the simpler reuse/launch path; challenge the author to take it. Why this is its own gate: deep seam-level rigor *launders* a wrong premise — the more carefully a parallel mechanism is specified, and the more its cites check out, the more settled it looks, yet a correct, well-grounded spec for a thing that **shouldn't exist** is still wrong. (OpenBrain lesson `4ea07f4d`: an affix-trainer spec proposed a second session engine + a `resolveCandidate` extraction; the app already had scoped sessions, so it collapsed to "launch the existing session, scoped" — caught by a non-coder's "isn't that just the start-session button?", not by two deeply-grounded reviews.) Your job is the simplest *sound* shape, not merely a correct one.
...
6b. **Render-contract changes enumerate the full gate set.** Any spec that changes how an existing capability renders (MCQ↔typed conversion, or splitting a shared `RENDER_CONTRACTS` row) must state whether a new ExerciseType is required — and if so, enumerate ALL compile-forced gates (ExerciseType union, RENDER_CONTRACTS, ContractInputShapes, projectBuilderInput, byType BUILDERS, exerciseSkeletonVariant, feedbackMapping switch) PLUS the non-compile-forced companions: the byType packager's audibleTexts wiring and buildFeedbackInput.ts's exerciseType-keyed conditionals (`answerLanguage`, `promptAudioUrl`) — those compile fine when missed and fail only at runtime. Under-enumeration = **WARNING**, or **CRITICAL** if the spec claims the list is complete. (OpenBrain lessons `6823ef85`, `43ee1ba0` — both under-enumerated.)
...
13b. **"Copy verbatim / mirror module A in B" instructions verified against B's actual conditions.** Before endorsing a spec that says "copy A's guard/pattern into B because B mirrors A's shape," diff A's and B's actual emission/branch conditions for every field the guard touches (grep both) — if B has ANY conditional branch A lacks, the copy must be scoped to that branch, not taken verbatim. Unverified "copy verbatim" = **WARNING**. (OpenBrain lesson `5003b4d8`: a verbatim guard copy would have broken two passing suites because the target module gates its audio cap on `hasAudio` and the source doesn't.)
