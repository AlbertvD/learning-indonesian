...
4. **Target architecture.** Open `docs/target-architecture.md`. Does the design's module placement match the target (`src/lib/<module>/...`) or does it grow in a stale location (`src/services/...`, ad-hoc `src/lib/foo.ts`)? Mismatch without reasoning = **WARNING**.
