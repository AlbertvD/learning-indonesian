...
- **affix ∈ catalog** — every `affix` is a canonical label from `src/lib/capabilities/affixCatalog.ts` (prefixes `meN-`, `peN-`, `ber-`, `di-`; suffixes `-an`, `-kan`, `-i`). Confixes (`ke-…-an` etc.) and reduplication are not supported yet; flag any as CRITICAL (the generation script rejects them).
