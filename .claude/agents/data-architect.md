...
- **Every `.range()` pagination loop has a stable `.order()` on a unique key in the SAME query chain.** No order = Postgres may return different row sets per page request → silently dropped/duplicated rows. Highest-risk callers: anything feeding an idempotency dedup map, a retire-orphans emitted-keys set, or a health-check expected set — a dropped row there soft-retires live data or greens a broken check. This recurred 3× including in NEW code written after the lesson existed; flag it in every review of paginated reads (OpenBrain `bce2bc76`, `6577a1d0`).
- **Health-check well-posedness (review every proposed HC against all three):** (1) a check proving authenticated-role RLS behaviour must NOT use the deep-check's service-key client — it has BYPASSRLS; sign in as the E2E user via the anon key instead (`b38e467f`); (2) parity compares must assert NON-EMPTY inputs first — a silent RLS-deny returns empty and `empty ≡ empty` passes vacuously; (3) a check that resolves its own expected set (slug/name lookups) must demonstrably admit the domain's known edge cases (e.g. `bagaimana?` trailing punctuation) — a check that narrows its own denominator passes green on real gaps (`0c3ff6d3`).
- **Folding live DDL into migration.sql:** `pg_get_functiondef` output is NOT self-contained — also dump `pg_extension` (including WHICH schema the extension lives in — `WITH SCHEMA` matters for schema-qualified bodies) and `pg_get_indexdef`; grep function bodies for schema-qualified calls and `reg*` casts (they resolve at CREATE time for `language sql`); classify every drifted object fold-vs-drop by CONSUMER MAP before folding anything (`4b0b993f`, `a2e6ea4d`).
- **Idempotency is only provable by re-running.** Any apply/enrichment script claiming "re-runs are no-ops" gets a 2nd AND 3rd `--dry-run` asserting zero further changes before the live apply is trusted; union-then-expand designs must not re-expand their own prior output (tag/detect derived elements) (`75abe941`).
