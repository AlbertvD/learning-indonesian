/**
 * capability-stage-legacy.ts
 *
 * Stage B publishing logic, extracted from the original
 * scripts/publish-approved-content.ts as part of the Phase 1 deep-module
 * refactor. Owns: grammar patterns, learning items + meanings + contexts,
 * exercise variants, cloze contexts, capability rows + exercise assets,
 * post-seed integrity verification.
 *
 * "Legacy" because Phase 2 of the pipeline rewrite extracts this into a
 * proper scripts/lib/pipeline/capability-stage/ module that reads from DB
 * instead of staging files. Until then, the publish-approved-content.ts
 * thin CLI invokes runLessonStage (Stage A) followed by
 * publishLegacyStageB (Stage B) to preserve the end-to-end publish loop.
 *
 * Stage A inline writes (lessons row + lesson_sections upserts) were
 * removed during the extraction — runLessonStage owns those now.
 */
...
/**
 * Stage B publish — runs after runLessonStage has already written the lesson
 * row + sections + page-blocks + audio_clips. Receives `lessonId` from the
 * runner so it doesn't have to re-find or re-upsert the lesson.
 */
export async function publishLegacyStageB(input: {
  lessonNumber: number
  lessonId: string
  dryRun: boolean
}): Promise<void> {
  const { lessonNumber, lessonId, dryRun } = input
  ...
  console.log(`\n${dryRun ? '[DRY RUN] ' : ''}Publishing Stage B for lesson ${lessonNumber}...`)

  try {
    if (lesson && lesson.sections.length > 0 && !dryRun) {
