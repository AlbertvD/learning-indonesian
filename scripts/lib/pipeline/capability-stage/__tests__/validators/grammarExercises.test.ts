...
describe('buildGrammarExerciseRow', () => {
  it('maps choose_correct_form_ex to typed columns (options stay [{id,text}], audit I2)', () => {
    const built = buildGrammarExerciseRow('choose_correct_form_ex', {
  ...
  it('maps choose_missing_word_ex with options as string[] (audit I2) and prefers answer_key correctOptionId', () => {
    const built = buildGrammarExerciseRow('choose_missing_word_ex', {
    ...
    expect(built?.table).toBe(GRAMMAR_EXERCISE_TABLE.choose_missing_word_ex)
  ...
  it('maps transform_sentence_ex (hint_text nullable, acceptable_answers from answer_key)', () => {
    const built = buildGrammarExerciseRow('transform_sentence_ex', {
  ...
  it('maps translate_sentence_ex (disallowed_shortcut_forms defaults to [])', () => {
    const built = buildGrammarExerciseRow('translate_sentence_ex', {
  ...
  it('returns null for a non-grammar exercise_type', () => {
    expect(buildGrammarExerciseRow('choose_meaning_ex', {}, {})).toBeNull()
  })
})
...
describe('validateGrammarExercises (CS13)', () => {
  it('passes valid candidates of all 4 types', () => {
    const findings = validateGrammarExercises([
      ...
      candidate('transform_sentence_ex', { sourceSentence: 's', transformationInstruction: 'i', acceptableAnswers: ['a'], explanationText: 'e' }),
      candidate('translate_sentence_ex', { sourceLanguageSentence: 's', requiredTargetPattern: 'p', acceptableAnswers: ['a'], explanationText: 'e' }),
      candidate('choose_missing_word_ex', { sentence: 's ___', translation: 't', options: ['a', 'b'], correctOptionId: 'a', explanationText: 'e' }),
    ])
    expect(findings).toEqual([])
  })

  it('flags choose_correct_form_ex when correct_option_id does not match any option id', () => {
    const findings = validateGrammarExercises([
      candidate('choose_correct_form_ex', { promptText: 'p', targetMeaning: 'm', options: [{ id: 'a', text: 'x' }, { id: 'b', text: 'y' }], correctOptionId: 'zzz', explanationText: 'e' }),
    ])
  ...
  it('flags choose_missing_word_ex with empty options / missing fields', () => {
    const findings = validateGrammarExercises([
      candidate('choose_missing_word_ex', { sentence: '', translation: 't', options: [], correctOptionId: '', explanationText: '' }),
    ])
    expect(findings).toHaveLength(1)
    expect(findings[0].gate).toBe('CS13')
  })

  it('flags transform_sentence_ex with empty acceptable_answers', () => {
    const findings = validateGrammarExercises([
      candidate('transform_sentence_ex', { sourceSentence: 's', transformationInstruction: 'i', acceptableAnswers: [], explanationText: 'e' }),
    ])
    expect(findings).toHaveLength(1)
  })
  ...
  it('skips non-grammar types and already-published candidates', () => {
    const findings = validateGrammarExercises([
      candidate('choose_meaning_ex', { anything: true }),
      candidate('choose_correct_form_ex', { /* malformed */ }, 'published'),
    ])
    expect(findings).toEqual([])
  })
  ...
  it('warns (severity warning, not error) when explanation_text is verbose (F4 conciseness)', () => {
    ...
    const findings = validateGrammarExercises([
      candidate('choose_correct_form_ex', { promptText: 'p', targetMeaning: 'm', options: [{ id: 'a', text: 'x' }, { id: 'b', text: 'y' }], correctOptionId: 'a', explanationText: verbose }),
    ])
    expect(findings).toHaveLength(1)
    expect(findings[0].gate).toBe('CS13')
  ...
  it('does not warn for a concise explanation (F4)', () => {
    const findings = validateGrammarExercises([
      candidate('choose_correct_form_ex', { promptText: 'p', targetMeaning: 'm', options: [{ id: 'a', text: 'x' }, { id: 'b', text: 'y' }], correctOptionId: 'a', explanationText: 'kort en bondig' }),
    ])
    expect(findings).toEqual([])
  })
})
