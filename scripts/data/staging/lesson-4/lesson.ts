// Lesson 4 — reverse-engineered from DB by reverse-engineer-staging.ts
export const lesson = {
  "title": "Les 4 - Di Hotel",
  "description": "Accommodation and dining in Indonesia. Grammar focus: YANG constructie (relative pronouns and emphasis patterns)",
  "level": "A1",
  "module_id": "module-1",
  "order_index": 4,
  "sections": [
    {
      "title": "Accommodatie (Cultuur)",
      "order_index": 0,
      "content": {
        "type": "text",
        "paragraphs": [
          "Les 4\nCULTUUR",
          "Sommigen brengen de nacht door onder een flonkerende tropenhemel op het witte strand van Bali. Het is romantisch en goedkoop, al moet je de voortdurende nieuwsgierigheid van stekende muggen, blaffende hondjes en nachtelijke vissers op de koop toe nemen. Mocht je toch in slaap vallen, dan ben je bij deze gewaarschuwd: om vijf uur 's ochtends gonst het strand van de bedrijvigheid. Van zee terugkerende vissers, hout sprokkelende vrouwen, spelende kinderen en dauwtrappende toeristen komen de eerste zonnestralen begroeten.",
          "Een andere mogelijkheid om de nacht door te brengen is het huren van een kamer in een \"penginapan\", vrij vertaald: \"een plaats waar je kunt overnachten\". Vaak betekent het inderdaad niets meer dan dat. Je krijgt er een slaapplaats aangewezen in een slaapzaal en wanneer je geluk hebt is er wellicht een afsluitbare tweepersoons-kamer te huur. Een gemeenschappelijk hurktoilet en 'mandi-kamer' die ettelijke dagen geen Hollandse schoonmaakbeurt hebben gekregen, zijn gebruikelijk. Ontbijt en andere maaltijden worden niet geserveerd, het beddegoed is niet altijd even nieuw en er wordt geen handdoek met zeep aangeboden. Echter over \u00e9\u00e9n ding kunnen zelfs 'budget travellers' niet kibbelen: de lage prijs. De kosten van een overnachting kunnen vari\u00ebren van Rp 7.000 tot Rp 20.000 (circa f 6,-- tot f 17,50). Natuurlijk zijn er vele nette en schone 'penginapans', doch over het algemeen zijn ze bedoeld voor de degene die op doorreis niet al te veel geld wil of kan besteden aan zijn nachtrust.",
          "De nettere slaapgelegenheden presenteren zich als \"losmen\" (van het Nederlandse 'logement'). Het beheer is meestal in handen van een familie die door verhuur van kamers een klein extra inkomen probeert te verwerven. Het zijn in feite goedkope hotels met dien verstande dat ze niet de daarbij behorende service verlenen. Doorgaans kun je er bijvoorbeeld geen ontbijt krijgen en uitgebreide drink- en eetfaciliteiten zijn eveneens afwezig. Meestal kan men wel voor een klein bedrag kleding laten wassen. De kwaliteit van de kamers en de hygi\u00ebne zijn sterk afhankelijk van de eigenaar waardoor we een grote verscheidenheid aantreffen: soms hoge prijzen en smoezelige kamers, dan weer brandschone badkamers en lage prijzen. Het verdient aanbeveling om altijd eerst te kijken voordat men erin trekt. De prijzen liggen tussen de Rp 15.000 - Rp 30.000.",
          "In juli 1988 heeft de Indonesische overheid voor Bali echter het verbod uitgevaardigd om toeristen in priv\u00e9huizen op te nemen teneinde een betere controle te krijgen op de toeristenstroom.",
          "Van betere kwaliteit zijn de 'guesthouses', al moet de reiziger ook hier uitkijken wat hij huurt. Over het algemeen is de hygi\u00ebne goed en zijn toilet en mandi-kamer schoon. Het zijn andere zaken waarop men dient te letten zoals: de ligging van de kamer, netheid, de aanwezigheid van een electrische ventilator (probeer hem eerst uit) of AC (-air conditioning) en voldoende verlichting."
        ]
      }
    },
    {
      "title": "Cultuur: Rijst in Indonesi\u00eb",
      "order_index": 1,
      "content": {
        "type": "text",
        "paragraphs": [
          "Reeds voor het begin van de christelijke jaartelling verbouwden de volkeren van Indonesi\u00eb rijst op zowel droge (lading) als natte (sawah) gronden. Natte rijstteelt geeft een hogere opbrengst maar is bijzonder arbeidsintensief. Daarnaast vereist het een uitgekiend bevloeiingssysteem. Dit heeft geleid tot de kenmerkende terrasbouw op onder andere Java en Bali.",
          "Rijst is het basisvoedsel van de overgrote meerderheid van de Indonesi\u00ebrs. Het is daarom niet verwonderlijk dat het Indonesisch verschillende termen voor rijst kent, al naar gelang de stadia van groei en verwerkerking van rijst.",
          "Wil men aangeven dat de rijst op een bepaalde wijze is bereid, dan voegt men nog een woord toe, zoals nasi putih (gekookte witte rijst), nasi goreng (gekookte rijst, daarna met kruiden gebakken) en nasi kuning (met kruiden geel gekookte rijst).",
          "Naast de in Nederland populaire 'droogkokende' rijstsoorten bestaan er ook kleefrijst- en paprijstsoorten, elk met hun specifieke smaak en aroma. De meeste rijstsoorten hebben een milde smaak en kunnen daarom goed dienen als 'neutraal' voedingsmiddel, waarbij de vol-aromatische bijgerechten volledig tot hun recht komen. Een normale Indonesische maaltijd bestaat uit rijst en twee tot drie andere gerechten."
        ]
      }
    },
    {
      "title": "Dialoog: Di Hotel",
      "order_index": 2,
      "content": {
        "type": "dialogue",
        "lines": [
          {
            "text": "Selamat siang, Mas. Kamar saya nomor berapa?",
            "speaker": "Ibu Dewi",
            "translation": ""
          },
          {
            "text": "Nomor 215, Bu. Di lantai 2.",
            "speaker": "Mas Wawan",
            "translation": ""
          },
          {
            "text": "Paspor Ibu di mana? Saya harus isi formulir hotel.",
            "speaker": "Mas Wawan",
            "translation": ""
          },
          {
            "text": "Ini paspor saya. Kunci saya di mana?",
            "speaker": "Ibu Dewi",
            "translation": ""
          },
          {
            "text": "Kunci yang kecil dan kuning ini kunci Ibu, silakan!",
            "speaker": "Mas Wawan",
            "translation": ""
          },
          {
            "text": "Kamar saya bagus. Kamar mandi ada di mana?",
            "speaker": "Ibu Dewi",
            "translation": ""
          },
          {
            "text": "Di sini, Bu. Handuk, sabun dan sikat gigi ada. Di situ bak air.",
            "speaker": "Mas Wawan",
            "translation": ""
          },
          {
            "text": "Air di bak untuk mandi.",
            "speaker": "Mas Wawan",
            "translation": ""
          },
          {
            "text": "Oke, terima kasih, ya Mas!",
            "speaker": "Ibu Dewi",
            "translation": ""
          },
          {
            "text": "Mas, saya ingin makan. Apa makanan yang enak?",
            "speaker": "Ibu Dewi",
            "translation": ""
          },
          {
            "text": "Oo, apa Ibu suka ayam pedas, nasi kuning dan lauk?",
            "speaker": "Mas Imran",
            "translation": ""
          },
          {
            "text": "Eeh... suka, tetapi saya ingin makan sate kambing, gado-gado dan nasi putih.",
            "speaker": "Ibu Dewi",
            "translation": ""
          },
          {
            "text": "Tambah krupuk juga dan segelas air putih, ya Mas.",
            "speaker": "Ibu Dewi",
            "translation": ""
          }
        ]
      }
    },
    {
      "title": "Grammatica: YANG - Constructie",
      "order_index": 3,
      "content": {
        "type": "grammar",
        "categories": [
          {
            "rules": [
              "Yang koppelt als betrekkelijk voornaamwoord een bijzin aan het voorgaande zelfstandig naamwoord.",
              "Yang staat altijd direct na het woord of de woordgroep waarnaar verwezen wordt.",
              "Na samengestelde zelfstandige naamwoorden (kamar mandi, tempat tidur) is yang verplicht.",
              "Na een bezittelijk voornaamwoord dient een bijvoeglijk naamwoord ook voorafgegaan te worden door yang."
            ],
            "title": "Yang als betrekkelijk voornaamwoord (die/dat)",
            "examples": [
              {
                "dutch": "Een banaan die te oud is, is niet lekker",
                "indonesian": "Pisang yang terlalu tua tidak enak"
              },
              {
                "dutch": "Dit is de Nederlander die in Bogor woont",
                "indonesian": "Ini orang Belanda yang tinggal di Bogor"
              },
              {
                "dutch": "Mijn kleine fiets is verdwenen",
                "indonesian": "Sepeda saya yang kecil hilang"
              }
            ]
          },
          {
            "rules": [
              "Yang kan bijvoeglijke naamwoorden, werkwoorden en andere woordsoorten omzetten naar zelfstandige naamwoorden.",
              "Vergelijkbaar met het Nederlandse 'de...' of 'degenen die...'."
            ],
            "title": "Yang maakt zelfstandige naamwoorden (nominalisering)",
            "examples": [
              {
                "dutch": "De dure (dingen) zijn mooi",
                "indonesian": "Yang mahal bagus"
              },
              {
                "dutch": "De grote is lekker",
                "indonesian": "Yang besar enak"
              },
              {
                "dutch": "Degenen die slapen zijn talrijk",
                "indonesian": "Yang tidur banyak"
              },
              {
                "dutch": "Degenen die naar de winkel willen moeten een taxi nemen",
                "indonesian": "Yang mau ke toko harus naik taksi"
              },
              {
                "dutch": "Deze twee zijn schoon",
                "indonesian": "Yang dua ini bersih"
              }
            ]
          },
          {
            "rules": [
              "Yang v\u00f3\u00f3r \u00e9\u00e9n bijvoeglijk naamwoord geeft nadruk aan dat eigenschap.",
              "Vergelijkbaar met het cursief zetten van een woord in het Nederlands."
            ],
            "title": "Yang bij \u00e9\u00e9n bijvoeglijk naamwoord (nadruk)",
            "examples": [
              {
                "dutch": "Het GROTE huis (nadruk op groot)",
                "indonesian": "Rumah yang besar"
              },
              {
                "dutch": "De SCHONE lepel",
                "indonesian": "Sendok yang bersih"
              },
              {
                "dutch": "De KLEINE stoel",
                "indonesian": "Kursi yang kecil"
              },
              {
                "dutch": "De WITTE badkamer",
                "indonesian": "Kamar mandi yang putih"
              }
            ]
          },
          {
            "rules": [
              "Bij ongelijke nadruk (\u00e9\u00e9n eigenschap benadrukt): [znw] [bijv.nw.] yang [benadrukt bijv.nw.]",
              "Bij gelijke nadruk (beide eigenschappen even belangrijk): [znw] yang [bijv.nw.] dan [bijv.nw.]"
            ],
            "title": "Yang bij twee bijvoeglijke naamwoorden",
            "examples": [
              {
                "dutch": "Het GELE grote huis (geel benadrukt)",
                "indonesian": "Rumah besar yang kuning"
              },
              {
                "dutch": "De schone kleine lepel (schoon benadrukt)",
                "indonesian": "Sendok kecil yang bersih"
              },
              {
                "dutch": "Het grote en gele huis (gelijke nadruk)",
                "indonesian": "Rumah yang besar dan kuning"
              },
              {
                "dutch": "De witte, zware kamerdeur (gelijke nadruk)",
                "indonesian": "Pintu kamar yang putih dan berat"
              }
            ]
          },
          {
            "rules": [
              "Ada yang... betekent 'er zijn er die...' of 'sommigen..., anderen...'.",
              "Wordt gebruikt om een groep te verdelen of een deel aan te duiden."
            ],
            "title": "Ada yang... \u2014 er zijn er die...",
            "examples": [
              {
                "dutch": "Er zijn er die uit Nederland komen en er zijn er die uit Duitsland komen",
                "indonesian": "Ada yang dari negeri Belanda dan ada yang dari negeri Jerman"
              }
            ]
          }
        ]
      }
    },
    {
      "title": "Oefeningen",
      "order_index": 4,
      "content": {
        "type": "exercises",
        "sections": [
          {
            "type": "grammar_drill",
            "items": [
              {
                "answer": "Rumah Jan yang besar bagus",
                "prompt": "Het huis van Jan dat groot is, is mooi"
              },
              {
                "answer": "Sepeda Fedi yang baru bersih",
                "prompt": "De fiets van Fedi die nieuw is, is schoon"
              },
              {
                "answer": "Di mana sekolah yang baru?",
                "prompt": "Waar is de nieuwe school?"
              },
              {
                "answer": "Buah yang mana ibu beli di pasar?",
                "prompt": "Welke vruchten heeft moeder op de markt gekocht?"
              },
              {
                "answer": "Mengapa kamar mandi yang besar belum bersih?",
                "prompt": "Waarom is de grote badkamer nog niet schoon?"
              },
              {
                "answer": "Krupuk yang enak itu saya beli di pasar",
                "prompt": "Die lekkere kroepoek heb ik op de markt gekocht"
              },
              {
                "answer": "Mobil yang kecil cepat",
                "prompt": "De kleine auto is snel"
              },
              {
                "answer": "Berapa handuk yang putih?",
                "prompt": "Hoeveel witte handdoeken zijn er?"
              },
              {
                "answer": "Di mana lemari yang baru itu?",
                "prompt": "Waar is die nieuwe kast?"
              }
            ],
            "title": "Oefening I",
            "instruction": "Vertaal en gebruik in elke zin yang."
          },
          {
            "type": "translation",
            "items": [
              {
                "answer": "Itu koper baru. Yang besar berat, tapi yang kecil ringan",
                "prompt": "Dat zijn nieuwe koffers. De grote is zwaar, maar de kleine is licht"
              },
              {
                "answer": "Di sana ada rumah. Yang baru besar, tapi yang lama kecil",
                "prompt": "Daar staan huizen. De nieuwe zijn groot, maar de oude zijn klein"
              },
              {
                "answer": "Ini lemari yang baru. Yang lama ada di kamar tidur",
                "prompt": "Hier is de nieuwe kast. De oude is in de slaapkamer"
              },
              {
                "answer": "Di mana anak-anak? Yang kecil ada di kamar mandi, yang besar ada di kamar makan",
                "prompt": "Waar zijn de kinderen? De kleintjes zijn in de badkamer, de groten zijn in de eetkamer"
              },
              {
                "answer": "Di pasar banyak orang. Ada yang beli buah dan ada yang beli sayur",
                "prompt": "Op de markt zijn veel mensen. Er zijn er die fruit kopen en er zijn er die groenten kopen"
              }
            ],
            "title": "Oefening II",
            "instruction": "Vertaal."
          },
          {
            "type": "grammar_drill",
            "items": [
              {
                "prompt": "4 fietsen",
                "answer": "Itu berapa sepeda? Itu empat sepeda."
              },
              {
                "prompt": "1.563 huizen",
                "answer": "Itu berapa rumah? Itu seribu lima ratus enam puluh tiga rumah."
              },
              {
                "prompt": "2.690 kippen",
                "answer": "Itu berapa ayam? Itu dua ribu enam ratus sembilan puluh ayam."
              },
              {
                "prompt": "187 vorken",
                "answer": "Itu berapa garpu? Itu seratus delapan puluh tujuh garpu."
              },
              {
                "prompt": "1.259 lepels",
                "answer": "Itu berapa sendok? Itu seribu dua ratus lima puluh sembilan sendok."
              },
              {
                "prompt": "375 vrienden",
                "answer": "Itu berapa teman? Itu tiga ratus tujuh puluh lima teman."
              },
              {
                "prompt": "193 kasten",
                "answer": "Itu berapa lemari? Itu seratus sembilan puluh tiga lemari."
              },
              {
                "prompt": "352 tafels",
                "answer": "Itu berapa meja? Itu tiga ratus lima puluh dua meja."
              },
              {
                "prompt": "23.675.890 kinderen",
                "answer": "Itu berapa anak? Itu dua puluh tiga juta enam ratus tujuh puluh lima ribu delapan ratus sembilan puluh anak."
              },
              {
                "prompt": "11.100 auto's",
                "answer": "Itu berapa mobil? Itu sebelas ribu seratus mobil."
              }
            ],
            "title": "Oefening III",
            "instruction": "Vraag en antwoord. Contoh: 3 koffers \u2192 Itu berapa koper? Itu tiga koper."
          },
          {
            "type": "grammar_drill",
            "items": [
              {
                "prompt": "keatas"
              },
              {
                "prompt": "terlalu"
              },
              {
                "prompt": "piring"
              },
              {
                "prompt": "ini"
              },
              {
                "prompt": "apa"
              },
              {
                "prompt": "tidak"
              },
              {
                "prompt": "dari"
              },
              {
                "prompt": "berapa"
              },
              {
                "prompt": "meja"
              },
              {
                "prompt": "dengan"
              },
              {
                "prompt": "buka"
              },
              {
                "prompt": "kiri"
              },
              {
                "prompt": "cari"
              },
              {
                "prompt": "siapa"
              },
              {
                "prompt": "cukup"
              },
              {
                "prompt": "sekarang"
              },
              {
                "prompt": "pelan-pelan"
              },
              {
                "prompt": "turun"
              },
              {
                "prompt": "naik"
              },
              {
                "prompt": "tempat duduk"
              }
            ],
            "title": "Oefening IV",
            "instruction": "Maak zinnen waarin het opgegeven woord voorkomt."
          }
        ]
      }
    },
    {
      "title": "Woordenlijst",
      "order_index": 5,
      "content": {
        "type": "vocabulary",
        "items": [
          {
            "dutch": "de jonge rijstplant",
            "indonesian": "bibit padi"
          },
          {
            "dutch": "volgroeide rijstplant met volle rijstaar, ongepelde rijst",
            "indonesian": "padi"
          },
          {
            "dutch": "ongepelde rijst",
            "indonesian": "gabah"
          },
          {
            "dutch": "gepelde ongekookte rijst",
            "indonesian": "beras"
          },
          {
            "dutch": "gekookte rijst",
            "indonesian": "nasi"
          },
          {
            "dutch": "gekookte witte rijst",
            "indonesian": "nasi putih"
          },
          {
            "dutch": "gekookte rijst, daarna met kruiden gebakken",
            "indonesian": "nasi goreng"
          },
          {
            "dutch": "met kruiden geel gekookte rijst",
            "indonesian": "nasi kuning"
          },
          {
            "dutch": "begeleiden",
            "indonesian": "mengantar"
          },
          {
            "dutch": "water",
            "indonesian": "air"
          },
          {
            "dutch": "drinken",
            "indonesian": "minum"
          },
          {
            "dutch": "drinkwater",
            "indonesian": "air putih"
          },
          {
            "dutch": "drank",
            "indonesian": "minuman"
          },
          {
            "dutch": "maar, echter",
            "indonesian": "tetapi"
          },
          {
            "dutch": "kind",
            "indonesian": "anak"
          },
          {
            "dutch": "waterbak",
            "indonesian": "bak air"
          },
          {
            "dutch": "heet (van smaak)",
            "indonesian": "pedas"
          },
          {
            "dutch": "schoon",
            "indonesian": "bersih"
          },
          {
            "dutch": "langzaam (aan)",
            "indonesian": "pelan-pelan"
          },
          {
            "dutch": "zitten",
            "indonesian": "duduk"
          },
          {
            "dutch": "bestellen",
            "indonesian": "pesan"
          },
          {
            "dutch": "formulier",
            "indonesian": "formulir"
          },
          {
            "dutch": "bord",
            "indonesian": "piring"
          },
          {
            "dutch": "groentegerecht",
            "indonesian": "gado-gado"
          },
          {
            "dutch": "verlangen, willen",
            "indonesian": "ingin"
          },
          {
            "dutch": "mes",
            "indonesian": "pisau"
          },
          {
            "dutch": "vork",
            "indonesian": "garpu"
          },
          {
            "dutch": "wit",
            "indonesian": "putih"
          },
          {
            "dutch": "glas",
            "indonesian": "gelas"
          },
          {
            "dutch": "netjes",
            "indonesian": "rapi"
          },
          {
            "dutch": "tand",
            "indonesian": "gigi"
          },
          {
            "dutch": "restaurant",
            "indonesian": "restoran"
          },
          {
            "dutch": "bakken, braden",
            "indonesian": "goreng"
          },
          {
            "dutch": "handdoek",
            "indonesian": "handuk"
          },
          {
            "dutch": "zeep",
            "indonesian": "sabun"
          },
          {
            "dutch": "weg, verdwenen (kwijt)",
            "indonesian": "hilang"
          },
          {
            "dutch": "pasta van Spaanse pepers",
            "indonesian": "sambal"
          },
          {
            "dutch": "sate",
            "indonesian": "sate"
          },
          {
            "dutch": "inhoud, (in)vullen",
            "indonesian": "isi"
          },
          {
            "dutch": "groente",
            "indonesian": "sayur"
          },
          {
            "dutch": "een bepaalde vrucht",
            "indonesian": "jambu air"
          },
          {
            "dutch": "school",
            "indonesian": "sekolah"
          },
          {
            "dutch": "Duitsland, Duits",
            "indonesian": "Jerman"
          },
          {
            "dutch": "vaarwel",
            "indonesian": "selamat tinggal"
          },
          {
            "dutch": "ook",
            "indonesian": "juga"
          },
          {
            "dutch": "lepel",
            "indonesian": "sendok"
          },
          {
            "dutch": "toilet, w.c.",
            "indonesian": "kamar kecil"
          },
          {
            "dutch": "na(dat), vervolgens",
            "indonesian": "setelah"
          },
          {
            "dutch": "eetkamer",
            "indonesian": "kamar makan"
          },
          {
            "dutch": "borstel",
            "indonesian": "sikat"
          },
          {
            "dutch": "badkamer",
            "indonesian": "kamar mandi"
          },
          {
            "dutch": "tandenborstel",
            "indonesian": "sikat gigi"
          },
          {
            "dutch": "geit",
            "indonesian": "kambing"
          },
          {
            "dutch": "houden van (lekker vinden)",
            "indonesian": "suka"
          },
          {
            "dutch": "sojasaus",
            "indonesian": "kecap"
          },
          {
            "dutch": "gast",
            "indonesian": "tamu"
          },
          {
            "dutch": "genoeg, voldaan",
            "indonesian": "kenyang"
          },
          {
            "dutch": "thee",
            "indonesian": "teh"
          },
          {
            "dutch": "koffie",
            "indonesian": "kopi"
          },
          {
            "dutch": "plaats",
            "indonesian": "tempat"
          },
          {
            "dutch": "zwarte koffie",
            "indonesian": "kopi pahit"
          },
          {
            "dutch": "zitplaats",
            "indonesian": "tempat duduk"
          },
          {
            "dutch": "zwarte koffie met suiker",
            "indonesian": "kopi tubruk"
          },
          {
            "dutch": "bed",
            "indonesian": "tempat tidur"
          },
          {
            "dutch": "te",
            "indonesian": "terlalu"
          },
          {
            "dutch": "kroepoek",
            "indonesian": "krupuk"
          },
          {
            "dutch": "klaar staan, gereed",
            "indonesian": "tersedia"
          },
          {
            "dutch": "sleutel",
            "indonesian": "kunci"
          },
          {
            "dutch": "slapen",
            "indonesian": "tidur"
          },
          {
            "dutch": "geel",
            "indonesian": "kuning"
          },
          {
            "dutch": "botsen",
            "indonesian": "tubruk"
          },
          {
            "dutch": "tekort, minder",
            "indonesian": "kurang"
          },
          {
            "dutch": "afdalen, uitstappen",
            "indonesian": "turun"
          },
          {
            "dutch": "die, dat",
            "indonesian": "yang"
          },
          {
            "dutch": "stoel",
            "indonesian": "kursi"
          },
          {
            "dutch": "vloer, verdieping",
            "indonesian": "lantai"
          },
          {
            "dutch": "bijgerechten",
            "indonesian": "lauk"
          },
          {
            "dutch": "kast",
            "indonesian": "lemari"
          },
          {
            "dutch": "voedsel",
            "indonesian": "makanan"
          },
          {
            "dutch": "baden",
            "indonesian": "mandi"
          },
          {
            "dutch": "goud (emas)",
            "indonesian": "mas"
          },
          {
            "dutch": "nog",
            "indonesian": "masih"
          },
          {
            "dutch": "tafel",
            "indonesian": "meja"
          }
        ]
      }
    },
    {
      "title": "Telwoorden",
      "order_index": 6,
      "content": {
        "type": "numbers",
        "items": [
          {
            "dutch": "100",
            "indonesian": "seratus"
          },
          {
            "dutch": "200",
            "indonesian": "dua ratus"
          },
          {
            "dutch": "300",
            "indonesian": "tiga ratus"
          },
          {
            "dutch": "400",
            "indonesian": "empat ratus"
          },
          {
            "dutch": "500",
            "indonesian": "lima ratus"
          },
          {
            "dutch": "600",
            "indonesian": "enam ratus"
          },
          {
            "dutch": "700",
            "indonesian": "tujuh ratus"
          },
          {
            "dutch": "800",
            "indonesian": "delapan ratus"
          },
          {
            "dutch": "900",
            "indonesian": "sembilan ratus"
          },
          {
            "dutch": "1.000",
            "indonesian": "seribu"
          },
          {
            "dutch": "2.000",
            "indonesian": "dua ribu"
          },
          {
            "dutch": "9.000",
            "indonesian": "sembilan ribu"
          },
          {
            "dutch": "10.000",
            "indonesian": "sepuluh ribu"
          },
          {
            "dutch": "1.000.000",
            "indonesian": "sejuta"
          },
          {
            "dutch": "1.000.000.000",
            "indonesian": "semiliar"
          },
          {
            "dutch": "1.000.000.000.000",
            "indonesian": "setriliun"
          }
        ]
      }
    }
  ]
}
