...
## Pronunciation primer · Shadowing · Pronunciation podcast

How the app teaches **pronunciation** — deliberately **not** as an ASR-graded "speaking" capability (ADR 0025). Indonesian is phonetic + tone-free, so learning to pronounce it is a **small, front-loaded, closed-list** problem (a handful of sounds a Dutch/English speaker systematically gets wrong), where *intelligible ≈ correct*. So there is **no ASR, no FSRS speaking capability, no backend, no per-use cost**; the `speaking` exercise type stays dead.

- **Pronunciation primer** — a consolidated, **L1-aware** reference + *perception* surface for the closed pitfall set: the sound/alphabet reference (consolidated from lesson 1) plus **minimal-pair audio** to *hear* the contrasts. Lives in **Leren** as a sibling of the Affix Trainer. Reference content, not scheduled by FSRS.
- **Shadowing** — ungraded *productive* practice: hear a native model utterance → say it → optionally **record yourself and A/B compare**. **Client-only** (MediaRecorder + playback), nothing persisted, no ASR, no FSRS. The model audio already exists (per-item TTS); shadowing adds the act of speaking against it.
- **Pronunciation podcast** — a **two-host, L1-specific** episode (one **NL** for Dutch speakers, one **EN** for English speakers — the pitfalls genuinely differ by L1) that walks through that L1's pitfalls with **native `id-ID` example words woven in**. Produced **authored-script → multi-voice/multi-language TTS → concatenation** (L1 host lines in an nl-NL/en-US voice; example words in a native id voice; not NotebookLM — pronunciation must be exact) — but as a **one-off production, not a reusable pipeline** (staff-engineer, 2026-06-29): a throwaway script (or manual synth-and-stitch) makes the two MP3s, since a generator for two rarely-changing files isn't worth building. The two are **twins**: one logical `texts` row with **two L1 audio faces** (`audio_path` NL + `audio_path_en` EN) even though the two scripts are not 1:1. (`audio_path_en` is **added to `texts` by this slice** — the dual-audio pattern itself comes from `lessons.audio_path_en`, the grammar podcast; it did not previously exist on `texts`. No `text_kind` discriminator: the readers degrade gracefully without one, and audio L1-routes off `audio_path_en` presence — data-architect + architect, 2026-06-30.) Distinct from the **Story podcast** (Indonesian narration, listening-only, ADR 0022) and the **grammar podcast** (NotebookLM, per-lesson).
...
## Morphology gloss (reader, exploratory)

The **exploratory morphological breakdown the Lezen reader shows on tap** (ADR 0024): for an affixed word, its **affix + function**, its **root + meaning**, and its **word family**, plus a link into the Affix Trainer. **Gloss-only** — it is a reading aid, mints **no capabilities**, and is wholly distinct from the **drilled** morphology (`affixed_form_pairs`, which *does* become capabilities, ADR 0020/0021, and stays curated/thin). Stored as a build-time pre-compute in **`item_morphology`** keyed by `normalized_text` (surface word) → `{root, affix, gloss_nl, gloss_en}`. The affix **function** is *derived at read time* (from the static `AFFIX_CATALOG`) and the **family** is a *join over shared `root`* — neither is stored, so they cannot drift. The derived form's **exact meaning** (`gloss_nl/_en`, e.g. `pembaca` = "lezer") *is* stored, because it is idiomatic/curated — non-compositional, not reconstructable from root + affix (lesson cd2aca2f); projected from `affixed_form_pairs.derived_gloss_*` + `learning_items.translation_*`. Populated (regenerable) from `affixed_form_pairs` (attested) + the deterministic `affixDecomposition` engine (strip-to-propose + derive-to-verify, the reverse of `affixDerivation`) over the reading corpus. Read by `lib/reading` only.
...
For the full model — the (source × capability × exercise) map, live counts, the known naming debt, and the **readable naming convention** (four layers `_src`/`_mode`/`_cap`/`_ex` + typed content concepts, no "artifact" umbrella) — see [`docs/current-system/capability-and-exercise-model.md`](docs/current-system/capability-and-exercise-model.md). **That convention is now fully live** — all four layers renamed (`_src`+`_cap` Phase A, `_ex` Phase B, `_mode` Phase C; shipped in code 2026-06-16, `docs/plans/2026-06-15-capability-naming-rename-plan.md`). The mode names are `recognise_mode`/`recall_mode`/`produce_mode` (`spoken_production` left as a dead-cluster value).
...
The **soft "known word" predicate used only by the Lezen reader** (`get_text_coverage`, PRD #299) to order texts by per-learner lexical coverage. Deliberately **softer than [[Mastered (capability)]]**: a word counts as *recognisable for reading* if its **recognition cap** (`capability_type = 'recognise_meaning_from_text_cap'`) has been **practiced** (`review_count ≥ 1`, the canonical `PRACTICED_MIN_REVIEWS` at `src/lib/lessons/overview.ts:16`). It is a **composite of two reused rules** — the `practiced` threshold (shared with `get_lessons_overview`) × the recognition-cap literal (shared with `get_collections_overview`'s "known word") — *not* a new third predicate. Both rules are parity-guarded (ADR 0015) by two assertions in `lessons-overview-mastery-parity.test.ts`. Rationale: 95% reading coverage needs words the learner can *recognise*, not *master* — `mastered` would undercount (you recognise far more words than you've mastered). The reader additionally counts a ~150-word function-word list as always-known and skips proper nouns (client-side, `lib/reading/`).

## Reading harvest

The **tap-to-learn action in the Lezen reader** (reader Phase 2 §4, ADR 0023/0024 plan): tapping a word shows its gloss, and an explicit **"+ leren"** confirm adds *that exact word* to the learner's spaced-repetition queue (suggest-then-confirm, never auto-add — avoids deck bloat). It is **membership-only**: confirming writes one row to **`learner_reading_harvest`** (`user_id`, `learning_item_id`; owner-RLS, learner-writable *directly* — a plain membership row, **not** `learner_capability_state`, so no security-definer RPC and no [[ADR 0004]] concern). Eligibility then rides the **existing collections gate-OR**: `lib/collections/membership.resolveActivatedMemberRefs` UNIONs each harvested item's `source_ref` (`learning_items/<normalized_text>`) into the set that feeds the session-builder `activatedCollectionRefs` gate (`pedagogy.ts:410`), and FSRS state is minted by the **existing review-commit path** on first review (`activation_source='review_processor'`) — **no new RPC, no `activation_source` CHECK change, no direct capability-state write, no new eligibility branch** (`lib/reading` writes membership; `lib/collections` reads it; `lib/reading` does not import `session-builder`). The harvest is offered whenever the gloss carries a `harvestableItemId`: for a **base** word that is its own id; for a **derived/affixed** word (morphology source, e.g. `mengembangkan`) it is the **root**'s id — derived forms aren't seeded as items (CONTEXT.md *items are primitives*), so "+ leren: *kembang*" adds the root the learner can actually study, and the affix rule is one tap away in the Affix Trainer. Base primitives are seeded via the **§5 pre-seed** (the residual reading-corpus words → `learning_items` through the hidden Common Words unit `lesson-999`); derived forms reach their root through the `item_morphology` decomposition (the `affixDecomposition` engine handles `meN-` and the nasal confixes `meN-…-kan`/`meN-…-i`). **Virtuous loop:** a harvested word practised once (`review_count ≥ 1`) then counts as [[Reading-coverage known]], raising future reading coverage.
