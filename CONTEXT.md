...
**Alternative-answer convention.** An answer-bearing artifact (`meaning:l1`, `accepted_answers:id`) may list several *equally acceptable* forms — e.g. "het is goedkoop; de prijs is laag". The **canonical "OR" separators are `;` and `/`** (`huis / woning`, `het is goedkoop; de prijs is laag`); the grader splits on these and accepts any listed form. **A comma is NOT a separator** — it is part of a single answer (a comma can occur inside a legitimate translation), so comma-delimited alternatives are a mis-encoding to be re-authored to `;`. The Capability Gate enforces this convention so a wrong separator never reaches the learner as one unmatchable blob.
