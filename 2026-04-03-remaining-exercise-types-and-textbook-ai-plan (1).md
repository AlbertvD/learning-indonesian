...
- Treat `recognition`, `typed recall`, and `cloze` as already implemented.
- Treat these as the text-based exercise families to implement and enable in the next cycle:
  - `cued_recall`
  - `contrast_pair`
  - `sentence_transformation`
  - `constrained_translation`
- Implement `cued_recall` as a lightweight reverse-direction multiple-choice variant that reuses the existing recognition-style option UI and live content model where possible.
...
**Step 3: Treat the April 3 docs as the source of truth for current inventory**

Ensure:
- the code catalog matches the April 3 plan and spec
- the March 30 plan remains a historical snapshot
- `cued_recall` is marked planned for this rollout, not already live
...
```bash
git add src/domain/learning/exerciseCatalog.ts src/__tests__/exerciseCatalog.test.ts
...
### Task 4A: Implement the Cued Recall Exercise
...
Cover:
- meaning prompt renders with a cue
- learner selects the correct Indonesian option from multiple choices
- answer submission records correctness and latency

**Step 2: Extend the payload types**

Add fields for:
- prompt meaning text
- optional cue text
- multiple-choice Indonesian options
- correct option id
- explanation text

**Step 3: Implement `CuedRecallExercise.tsx`**

Support:
- reverse-direction prompt display
- optional cue display
- multiple-choice answer selection
- feedback with target reveal

Implementation constraint:
- reuse the existing recognition-style option UI and distractor-generation path where possible instead of introducing a separate heavy content pipeline
...
```bash
npm test -- cuedRecallExercise
```

Expected:
- cued recall works as a lightweight bridge between recognition and typed recall

**Step 6: Commit**

```bash
git add src/components/exercises/CuedRecallExercise.tsx src/components/exercises/ExerciseShell.tsx src/types/learning.ts src/__tests__/cuedRecallExercise.test.tsx
git commit -m "feat: add cued recall exercise"
```
...
- supported reverse-direction vocabulary recall can prefer `cued_recall` before `typed_recall`
...
- reverse-direction support ladder for vocabulary recall
- grammar-pattern complexity
- confusable-pattern interleaving
- exercise availability registry gating
- feature-flag gating for newly added exercise families
- branch scheduling by content path:
  - `cued_recall` may use the existing live retention-first content model
...
2. Task 4A: add cued recall
3. Task 2: add staging schema
4. Task 3: build textbook intake
5. Task 4: add reviewed AI generation
6. Task 9: add review UI
7. Task 5: contrast pair
8. Task 6: sentence transformation
9. Task 7: constrained translation
10. Task 8: speaking contracts only
11. Task 10: session-builder selection rules
12. Task 11: verification and deferred-work note
