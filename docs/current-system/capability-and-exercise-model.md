...
This is the **documented target vocabulary only.** The live enums (`CapabilityType`, `ExerciseType`, `source_kind`) still use the current names. Adopting it is a **migration that rewrites `canonical_key`** — which keys every learner's FSRS row — so it is an **architect + data-architect plan**, not a doc edit. The deeper §7 structural questions are now **settled** (§7 box): each (direction × modality × level) is its own capability — so `recognise_form_from_meaning_cap` is real, and the three `produce_form_*` stay separate. The migration's remaining structural work is the cleanup the resolution implies: relabel `l1_to_id_choice` as `recognition`-level, and forbid the cross-level renders (cloze and pattern produce-vs-recognition).
