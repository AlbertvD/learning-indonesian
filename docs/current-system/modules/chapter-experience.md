...
- **Exit to overview:** the sticky nav carries a persistent "Terug naar lessen" back link (`classes.backLink`) → `/leren`, present on every chapter. The global chrome (desktop sidebar / mobile bottom-nav) also reaches `/leren`, but a mode-switch tab doesn't read as "back" from inside an immersive lesson, so the reader owns its own up-affordance (added 2026-07-07 after the rollout — users reported no way back out of a lesson).
