...
| `affixCatalog.ts` | 145 | The controlled vocabulary of Indonesian affixes + curated curriculum metadata (capstone item A) — `AFFIX_CATALOG` (per-affix `affix`/`affixType`/`gloss`/`rank` (teaching sequence)/`cefrLevel`/`allomorphClasses`/`composition`), `AFFIX_SET` + `isCatalogAffix` (the `affix ∈ catalog` membership gate the pipeline asserts), `affixCatalogEntry`, `allomorphClassesFor`, `distractorAffixes`. The sole pipeline↔runtime shared seam for affix facts: the pipeline validator (`validators/affixedFormPairs.ts`) + HC31 + `generate-morphology-patterns.ts` **deep-import** it (build scripts may not pull a runtime barrel — target-architecture.md:1159); runtime consumers (the byType packagers + the Affix Trainer in `lib/morphology`) import it **via the barrel**. A code constant, not a DB table. |
| `affixDerivation.ts` | 320 | `deriveAffixedForm(root, affix)` — the deterministic affix composition engine (ADR 0019); reads each catalog entry's `composition` recipe to build the surface form. `blankDerivedInCarrier(carrier, derived)` blanks the derived form inside a carrier sentence for cloze. `UnsupportedAffixError` when an affix has no recipe. Types `DerivedAffixedForm`, `MorphologyRoot`. Authoring-time engine behind `generate-morphology-patterns.ts`; also read at runtime by the `decompose_word_ex`/`type_form_ex` packagers. |
...
**2026-06-16 — grammar production split SHIPPED (ADR 0017).** Each grammar pattern now emits **three** capabilities: `recognise_grammar_pattern_cap` (→ `choose_missing_word_ex` cloze only), `contrast_grammar_pattern_cap` (→ `choose_correct_form_ex`), and the new **`produce_grammar_pattern_cap`** (→ `transform_sentence_ex` + `translate_sentence_ex`), gated `recognise → contrast → produce` by `prerequisiteKeys`. The two production exercises moved off `recognise_grammar_pattern_cap` (resolving the §7.6 level-purity residual the Phase-A rename note flagged as "not folded in"). One shared renderability predicate (`satellitePresence.ts`) gained a third arm; mastery folds `produce` into the existing `pattern_use` dimension (no new `MasteryDimension`) and surfaces a third "Produceren" progress chip. Additive cutover — no migration (`capability_type` is bare `text`), the 1,471 authored grammar-exercise rows repointed by routing alone. Live: recognise=contrast=produce=97, junction 97, HC19/HC20/**HC30** green.

**2026-06-16 — §8 capability-naming rename, Phase A (`_src` + `_cap`).** `CapabilitySourceKind` and `CapabilityType` adopted the readable §8 convention (`capabilityTypes.ts`): source kinds → `vocabulary_src`/`grammar_pattern_src`/`dialogue_line_src`/`podcast_segment_src`/`podcast_phrase_src`/`word_form_pair_src`; capability types → the `operation_object_from_stimulus` names (`recognise_meaning_from_text_cap`, `recall_meaning_from_text_cap`, `recognise_form_from_meaning_cap`, `produce_form_from_meaning_cap`, `produce_form_from_context_cap`, `recognise_meaning_from_audio_cap`, `produce_form_from_audio_cap`, `recognise_gist_from_audio_cap`, `recognise_grammar_pattern_cap`, `contrast_grammar_pattern_cap`, `recognise_word_form_link_cap`, `produce_derived_form_cap`). `buildCanonicalKey` re-emits every key with the new segments; the `source_kind` CHECK in `scripts/migration.sql` was guard-rewritten. **Two surfaces tsc could not catch** (scripts/ is untyped + loosely-typed Sets): `masteryModel.GRAMMAR_SOURCE_KINDS`/`funnelBucket` (a `new Set([...])` of source kinds) and `check-capability-health.ts`'s `?? 'item'` default — both fixed, caught by the runtime test suite. `ExerciseType` (`_ex`) and `SkillType` (`_mode`) keep the former names pending Phases B/C (`docs/plans/2026-06-15-capability-naming-rename-plan.md`). The §7.6 level-purity residual is **not** folded in (it would mint a production-grammar capability with no §8 name — separate decision). `podcast_phrase` → `podcast_phrase_src` (renamed, retirement deferred). Historical entries below keep their period-accurate former names.
