...
| `graduation.ts` | ~100 | **Vocab graduation** (added 2026-07-08; extended 2026-07-09 PR-A for the `#2 ← #6` lane — `docs/plans/2026-07-09-vocab-four-card-ladder.md` §2.4). Exports the pure `suppressGraduatedVocabDue(orderedDue, capabilitiesByKey, schedulerRows)` — a scaffold→successors map (`GRADUATION_RULES`) removes vocab #1 (`recognise_meaning_from_text_cap`) AND #2 (`recognise_form_from_meaning_cap`) due entries once the same-`source_ref` #6 (`produce_form_from_meaning_cap`) sibling reaches `hasMasteryStrength` (`lib/analytics/mastery/mastered.ts`). Applied in `builder.ts` immediately after `getDueCapabilities`, before the grammar due-floor / size cut. #1's successor set stays the single shipped type #6 in PR-A — the `#1 ← (#3′ ∨ #6)` OR-repoint is PR-B (sequencing constraint, §2.4). See §3.2, §4, `docs/plans/2026-07-08-vocab-mode-set-reduction-and-graduation.md` §4.2, `docs/plans/2026-07-09-vocab-four-card-ladder.md` §2.4, ADR 0027. |
...
- **Vocab graduation retires vocab scaffold cards from DUE SCHEDULING once their production-frontier successor reaches mastery strength — the four-card model (`#1 ← #6`, `#2 ← #6`; PR-A adds the second lane, `docs/plans/2026-07-09-vocab-four-card-ladder.md` §2.4)** (`#1` rule added 2026-07-08, `docs/plans/2026-07-08-vocab-mode-set-reduction-and-graduation.md` §4.2; `#2` rule added 2026-07-09; ADR 0027-vocabulary-mode-set-bounded.md, amended). `suppressGraduatedVocabDue` (`graduation.ts`) is applied to the full overdue-ordered due list immediately after `getDueCapabilities` — **before** the grammar due-floor / size cut and before `backlogDueCount` is taken (§3.2 Step B) — so a graduated word's dropped scaffold review feeds both `dueCount` (→ `openSlots`, `loadBudget.ts:24`) and the Home backlog insight equally. Internally a `GRADUATION_RULES` map (scaffold capability type → successor type(s)) generalizes the single 2026-07-08 rule so a future scaffold or an OR-successor set (PR-B's `#1 ← (#3′ ∨ #6)` repoint) is one map entry, not a new code path; PR-A intentionally keeps #1's successor set as the single shipped type #6 — repointing #1 at #3′ while #3′ is still the old MCQ format would graduate #1 on MCQ-earned strength, then flicker when PR-B hardens the format (sequencing constraint, four-card-ladder spec §2.4). Uses `hasMasteryStrength` (`lib/analytics/mastery/mastered.ts`), **never** `isCapabilityMastered`: the latter's 30-day recency window would flicker a mature #6 card in and out of "mastered" between its own (increasingly spaced-out) reviews, oscillating the graduation instead of converging it. Stateless by construction: a lapse on #6 (`consecutiveFailureCount > 0`) breaks the strength predicate on the very next build and the scaffold reappears in the due list on its own — there is no stored "graduated" flag to reconcile. `#3` (`recognise_meaning_from_audio_cap`, aural) and `#6` itself are **never** suppressed; only `#1`/`#2` due entries are ever removed, and only for the vocab family (`sourceKind === 'vocabulary_src'`) — every other branch (missing capability projection, missing successor scheduler state, a non-vocab source kind, or #3/#6 itself) falls through unsuppressed by construction. **Deliberately has NO introduction-suppression counterpart** (a removed design decision — plan §4.3): an earlier draft also suppressed *introducing* #1 for a placement-seeded word whose #6 was already strong, but that would leave #1's key permanently out of `satisfiedKeys`, blocking a morphology root's derived-form cross-prereq (`affixedCapabilities.ts:49-57` + `pedagogy.ts:320`) for one saved review's worth of value. The system self-resolves without it: #1/#2 are introduced once, their first success populates `satisfiedKeys`, and due-suppression retires them from then on. Do not "complete" this by adding intro-suppression later without re-solving that prereq edge.
