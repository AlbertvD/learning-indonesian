...
| `graduation.ts` | ~65 | **Vocab graduation** (added 2026-07-08). Exports the pure `suppressGraduatedVocabDue(orderedDue, capabilitiesByKey, schedulerRows)` — removes vocab #1 (`recognise_meaning_from_text_cap`) due entries once the same-`source_ref` #6 (`produce_form_from_meaning_cap`) sibling reaches `hasMasteryStrength` (`lib/analytics/mastery/mastered.ts`). Applied in `builder.ts` immediately after `getDueCapabilities`, before the grammar due-floor / size cut. See §3.2, §4, `docs/plans/2026-07-08-vocab-mode-set-reduction-and-graduation.md` §4.2, ADR 0027. |
...
- **Vocab graduation retires vocab #1 (`recognise_meaning_from_text_cap`) from DUE SCHEDULING once its same-`source_ref` #6 sibling (`produce_form_from_meaning_cap`) reaches mastery strength** (added 2026-07-08, `docs/plans/2026-07-08-vocab-mode-set-reduction-and-graduation.md` §4.2, ADR 0027-vocabulary-mode-set-bounded.md). `suppressGraduatedVocabDue` (`graduation.ts`) is applied to the full overdue-ordered due list immediately after `getDueCapabilities` — **before** the grammar due-floor / size cut and before `backlogDueCount` is taken (§3.2 Step B) — so a graduated word's dropped #1 review feeds both `dueCount` (→ `openSlots`, `loadBudget.ts:24`) and the Home backlog insight equally. Uses `hasMasteryStrength` (`lib/analytics/mastery/mastered.ts`), **never** `isCapabilityMastered`: the latter's 30-day recency window would flicker a mature #6 card in and out of "mastered" between its own (increasingly spaced-out) reviews, oscillating the graduation instead of converging it. Stateless by construction: a lapse on #6 (`consecutiveFailureCount > 0`) breaks the strength predicate on the very next build and #1 reappears in the due list on its own — there is no stored "graduated" flag to reconcile. `#3` (`recognise_meaning_from_audio_cap`, aural) and `#6` itself are **never** suppressed; only `#1` due entries are ever removed, and only for the vocab family (`sourceKind === 'vocabulary_src'`) — every other branch (missing capability projection, missing #6 scheduler state, a non-vocab source kind, or any other vocab mode) falls through unsuppressed by construction. **Deliberately has NO introduction-suppression counterpart** (a removed design decision — plan §4.3): an earlier draft also suppressed *introducing* #1 for a placement-seeded word whose #6 was already strong, but that would leave #1's key permanently out of `satisfiedKeys`, blocking a morphology root's derived-form cross-prereq (`affixedCapabilities.ts:49-57` + `pedagogy.ts:320`) for one saved review's worth of value. The system self-resolves without it: #1 is introduced once, its first success populates `satisfiedKeys`, and due-suppression retires it from then on. Do not "complete" this by adding intro-suppression later without re-solving that prereq edge.
