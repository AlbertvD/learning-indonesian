...
| `graduation.ts` | ~120 | **Vocab graduation** (added 2026-07-08; extended 2026-07-09 PR-A for the `#2 ← #6` lane, then PR-B for the `#1 ← (#3′ ∨ #6)` OR-repoint — `docs/plans/2026-07-09-vocab-four-card-ladder.md` §2.4). Exports the pure `suppressGraduatedVocabDue(orderedDue, capabilitiesByKey, schedulerRows)` — a scaffold→successors map (`GRADUATION_RULES`) removes vocab #1 (`recognise_meaning_from_text_cap`) AND #2 (`recognise_form_from_meaning_cap`) due entries once ANY qualifying same-`source_ref` successor reaches `hasMasteryStrength` (`lib/analytics/mastery/mastered.ts`) — #2's sole successor is #6 (`produce_form_from_meaning_cap`); #1's successors are #3′ (`recognise_meaning_from_audio_cap`, ear-only typed recall since PR-B) OR #6. Applied in `builder.ts` immediately after `getDueCapabilities`, before the grammar due-floor / size cut. See §3.2, §4, `docs/plans/2026-07-08-vocab-mode-set-reduction-and-graduation.md` §4.2, `docs/plans/2026-07-09-vocab-four-card-ladder.md` §2.4, ADR 0027. |
...
**Step B — pass 1: due capabilities** (`builder.ts`). `getDueCapabilities` (`dueFilter.ts`) is called with `limit = MAX_SAFE_INTEGER` to get the **full** overdue-ordered due list (the projection stays family-agnostic — it migrates to `lib/analytics/upcoming/` per the target). The result is immediately run through `suppressGraduatedVocabDue` (`graduation.ts`, added 2026-07-08, extended by PR-A + PR-B) — vocab-graduation due-suppression; see §4 for the full rule. In **standard** mode the builder then applies `reserveGrammarDueFloor(orderedDue, limit, familyOf)` (`compose.ts`) — guaranteeing grammar family up to `floor(limit * GRAMMAR_DUE_FLOOR_FRACTION)` of the due slots before the size cut, so grammar isn't drowned by the vocab-majority due pool (grammar was ~6% of the owner's reviews, 2026-07-05 audit). Lesson-scoped modes keep the exact legacy top-N cut. `familyOf` resolves family from the loaded `capabilitiesByKey` snapshot — family knowledge never enters the due projection. Result is filtered to lesson scope when applicable, then each due item is resolved via the shared `resolveCandidate` helper.
...
- **Vocab graduation retires vocab scaffold cards from DUE SCHEDULING once a qualifying typed successor reaches mastery strength — the four-card model (`#1 ← (#3′ ∨ #6)`, `#2 ← #6`; `docs/plans/2026-07-09-vocab-four-card-ladder.md` §2.4)** (`#1 ← #6` rule added 2026-07-08, `docs/plans/2026-07-08-vocab-mode-set-reduction-and-graduation.md` §4.2; `#2 ← #6` rule added 2026-07-09 PR-A; `#1`'s OR-repoint to `#1 ← (#3′ ∨ #6)` added 2026-07-09 PR-B, once #3′ itself became a typed recall card (§2.3) — repointing earlier would have graduated #1 on MCQ-earned strength, then flickered when #3′ hardened; ADR 0027-vocabulary-mode-set-bounded.md, amended). `suppressGraduatedVocabDue` (`graduation.ts`) is applied to the full overdue-ordered due list immediately after `getDueCapabilities` — **before** the grammar due-floor / size cut and before `backlogDueCount` is taken (§3.2 Step B) — so a graduated word's dropped scaffold review feeds both `dueCount` (→ `openSlots`, `loadBudget.ts:24`) and the Home backlog insight equally. Internally a `GRADUATION_RULES` map (scaffold capability type → successor type(s), the successor set unioned via `.some(...)` for the OR) generalizes what would otherwise be copy-pasted branches, so a future scaffold or OR-successor set is one map entry, not a new code path. The `#1` OR is load-bearing for listening-disabled users: `listeningFilter.ts` strips #3′ from their snapshot, so a #3′-only trigger would leave #1 — a cued MCQ — as their lifelong card, contradicting the four-card model's thesis that every lifelong card must be uncued; with the OR their #1 graduates via #6 instead (§2.6). Uses `hasMasteryStrength` (`lib/analytics/mastery/mastered.ts`), **never** `isCapabilityMastered`: the latter's 30-day recency window would flicker a mature successor card in and out of "mastered" between its own (increasingly spaced-out) reviews, oscillating the graduation instead of converging it. Stateless by construction: a lapse on the triggering successor (`consecutiveFailureCount > 0`) breaks the strength predicate on the very next build and the scaffold reappears in the due list on its own — there is no stored "graduated" flag to reconcile (an OR successor lapsing while the other still qualifies leaves the scaffold suppressed — OR, not AND). `#3′` and `#6` themselves are **never** suppressed; only `#1`/`#2` due entries are ever removed, and only for the vocab family (`sourceKind === 'vocabulary_src'`) — every other branch (missing capability projection, missing successor scheduler state, a non-vocab source kind, or #3′/#6 itself) falls through unsuppressed by construction. **Deliberately has NO introduction-suppression counterpart** (a removed design decision — plan §4.3): an earlier draft also suppressed *introducing* #1 for a placement-seeded word whose #6 was already strong, but that would leave #1's key permanently out of `satisfiedKeys`, blocking a morphology root's derived-form cross-prereq (`affixedCapabilities.ts:49-57` + `pedagogy.ts:320`) for one saved review's worth of value. The system self-resolves without it: #1/#2 are introduced once, their first success populates `satisfiedKeys`, and due-suppression retires them from then on. Do not "complete" this by adding intro-suppression later without re-solving that prereq edge.
...
- `lib/analytics/mastery/mastered.ts` — `graduation.ts` imports `hasMasteryStrength` (added 2026-07-08 alongside `isCapabilityMastered`) as the recency-free graduation trigger; see §4. Not a two-way seam — `mastered.ts` has no session-builder import.
...
**Vocab-graduation analytics counterpart is a separate slice, not yet shipped.** `get_lessons_overview`'s mastered-percentage numerator does not yet know about graduation (Slice 3 of `docs/plans/2026-07-08-vocab-mode-set-reduction-and-graduation.md`, unmerged as of this spec's `last_verified_against_code` date) — until it ships, a graduated word's #1 ages out of that RPC's numerator after 30 days while still counting in the denominator, so lesson `% mastered` can dip even though the session builder correctly stopped scheduling it. This module's own behaviour (due-suppression) is unaffected either way.
...
- **Vocab-graduation analytics (mastered-percentage numerator subsumption + stability-scaled recency window).** Slice 3 of `docs/plans/2026-07-08-vocab-mode-set-reduction-and-graduation.md` — SQL-side (`get_lessons_overview`), not this module. See §6.
