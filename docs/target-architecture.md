...
- **The capability-related tables** (`learning_capabilities`, `learner_capability_state`, `capability_review_events`, `capability_resolution_failure_events`, `capability_aliases`, `capability_content_units`) — central to the architecture. (`capability_artifacts` was dropped in Slice 4b / #102 — superseded by the typed satellite tables per data-model-target Decision A.)
...
| Fold capability + content-units schema into `migration.sql` | Medium | The capability tables (`learning_capabilities`, `capability_aliases`, `learner_capability_state`, `capability_review_events`, `content_units`, `capability_content_units`, `capability_resolution_failure_events`) plus their indexes / grants / RPCs currently live in `scripts/migrations/2026-04-25-*` + `2026-05-02-*` standalone files. (`capability_artifacts` was dropped in Slice 4b / #102 and `lesson_page_blocks` in PR 5 — their teardowns are authored in `migration.sql`.) Fresh DB rebuilds need both files + `migration.sql` until folded. The 2026-05-08 bulk-drop fix addressed the immediate regression (per-policy `drop if exists; create` pairs in `migration.sql` no longer wipe these) but not the structural source-of-truth gap. |
