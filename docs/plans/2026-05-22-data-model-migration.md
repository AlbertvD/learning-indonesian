...
**Target:** Add typed columns on `lesson_sections` + 5 child tables **alongside the retained `content jsonb` blob** (the blob is kept — see §9.1 / §10.2). `lesson_dialogue_lines` already exists from PR 2.

**These typed tables are dual-purpose (ADR 0011):** the lesson reader's structured source *and* the **capability-stage contract** — so they carry the fields the Capability Stage needs (stable item identity + all three languages), not just what the renderer reads. PR 6 establishes them; the capability-stage redesign (#98/#99) consumes them.
...
- `lesson_section_item_rows` — per vocab/expression/number item: `source_item_ref` (stable capability identity), `item_type` (word/phrase/sentence), `indonesian_text`, `l1_translation` (Dutch, from the book), `l2_translation` (English, **generated by the Lesson Stage** — lesson material, not in the book)
...
`lesson-stage/validators/sectionShape.ts`: assert per-`section_kind` field applicability; for item rows assert `source_item_ref`, `item_type`, `indonesian_text`, `l1_translation`, and `l2_translation` non-null. CRITICAL if not. (The `l2_translation` non-null gate mirrors PR 1's `translation_en` enforcement — now owned by Stage A.)
...
| `lesson_section_item_rows` | `lesson-stage/runner.ts` + EN enricher | per-lesson `Page.tsx` × 9 + `coverageService.ts` + **capability-stage (post-PR-6, #98/#99)** | DB NOT NULL + UNIQUE(section_id, display_order) + UNIQUE(lesson_id, source_item_ref) + `sectionShape.ts` (`l2_translation` + `source_item_ref` non-null) |
