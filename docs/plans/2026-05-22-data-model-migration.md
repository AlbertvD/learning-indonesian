...
| **PR 6** | Lesson sections (Stage A): typed capability contract + re-publish | PR 5 | ✅ shipped — #108. 4 typed tables + section_kind/source_section_ref + dialogue/grammar NL+EN; EN enricher relocated to lesson-stage; write-only (reader is #98/#99). Re-published 6/9 (1,2,3,4,6,9); L5/7/8 pre-existing lint-blocked (dialogue-cloze gaps) → typed rows pend that cloze authoring fix; see §9 |
