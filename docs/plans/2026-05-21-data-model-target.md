---
status: superseded
superseded_by: 2026-05-22-data-model-migration.md (+ ADR 0011/0012)
...
> **⚠ Superseded in part — written 2026-05-21, before ADR 0011, ADR 0012, and the PR 5 page-block retirement.** Treat the schema *shapes* as the proposal of record, but re-verify these decisions against the ADRs before implementing:
> - **ADR 0011 (capability content DB-authoritative after seeding):** the Capability Stage is a generator/seeder, **not** a continuous projector. Phrases below like "derived at projection time" / "regenerated by the pipeline" reflect the pre-0011 projection model.
...
- **ADR 0011:** Capability content is DB-authoritative after seeding. The Capability Stage seeds each capability once; routine re-runs are idempotent/additive-only (skip-if-exists); corrections live in the DB. It is a generator/seeder, **not** a continuous projector.
- **ADR 0012:** Stage responsibilities — the Lesson Stage owns ingestion + all learner-facing enrichment (incl. NL + EN translations) and writes lesson content to the DB; the Capability Stage reads that content **only from the DB (never disk)** and generates everything capability-side. Dividing line: *what the learner reads* (Lesson Stage) vs. *what feeds generation/scheduling* (Capability Stage).
...
- `learning_capabilities.artifact_fingerprint` is retired — readiness is derived from "do the upstream rows the contract needs exist + are approved" at **seed time** (ADR 0011 — the Capability Stage seeds readiness once; it is not re-derived on a routine re-publish).
...
> **Refined 2026-05-25 (ADR 0012 — see migration plan §9).** The typed satellites are the **capability-stage contract**: every *capability-feeding* section gets a typed table so the Capability Stage reads structured rows, **not prose (no interpreter)**. **Built:** `lesson_section_item_rows` (→ `item` caps), `lesson_section_grammar_categories` + `lesson_section_grammar_topics` (→ `pattern` caps), and **`lesson_section_affixed_pairs`** (→ `affixed_form_pair` caps — new; the DB form of `morphology-patterns.ts`, carrying `root_text`/`derived_text`/`affix`/`allomorph_rule`), plus `section_kind` + `source_section_ref` on the parent. **Not built (display-only → blob):** `lesson_section_pronunciation_letters`, `lesson_section_exercise_groups`, and the 8 parent display columns (`intro`/`paragraphs`/`word_order`/`note`/`setup`/`closing`/`table_title`/`reference_payload`) — pronunciation, book exercises, and reading/culture have no capability consumer, and the bespoke `Page.tsx` render them from `content.json`. The bespoke-field gap (`examples`/`spelling`/`sentences`/`borobudur_levels`) stays in the retained blob. Numbers are capability-feeding both ways: named numbers (0–20 + place-value landmarks) → `lesson_section_item_rows` (vocab `item` caps), and the number-formation series → the drilled `belas-numbers` `pattern`. (Earlier in this session the grammar tables were wrongly grouped with the display-only drops; this note corrects that — grammar/morphology feed capabilities and are typed.)
...
create table indonesian.lesson_section_item_rows (
  ...
  display_order integer not null,
  source_item_ref text not null,                  -- 'lesson-N/section-M/item-K' — per-occurrence lesson-side identity (the DB replacement for staging learning-items.ts). NOT the item cap's source_ref: item caps key on 'learning_items/<text>' (global dedup), so the Capability Stage reduces these rows by indonesian_text → learning_items.normalized_text. (Contrast lesson_dialogue_lines.source_line_ref, which IS the dialogue cap source_ref — dialogue caps are per-occurrence.) Stable across re-authoring; display_order is not.
...
create table indonesian.content_units (
  ...
  -- content_units are derived from canonical content and re-seedable (ADR 0011:
  -- additive-only / skip-if-exists, not blind-regenerated) so
...
**Amendment (2026-06-04, surfaced by Slice 5 / #147) — the `grammar_pattern` unit identity source.** Decision E keeps `content_units` as a slug-identity table but did NOT secure a DB-resident identity source for the `grammar_pattern` unit kind. The legacy staging builder keyed grammar units off the **curated `pattern.slug`** in `scripts/data/staging/lesson-N/grammar-patterns.ts` (e.g. `bukan-negation`) — a field the typed tables never captured, so it is **irreproducible once staging is retired**. Live DB confirms three divergent grammar identity strings: grammar content_unit `pattern-bukan-negation`, grammar capability `source_ref` `lesson-6/pattern-l6-bukan-ontkenning…` (the pattern path's `l{N}-${stableSlug(category.title)}`), and the raw `category.title`. **Resolution: the `grammar_pattern` content_unit identity derives from the pattern path's `l{N}-${stableSlug(category.title)}` slug — the same DB-resident key the grammar *capability* uses — so `content_unit.source_ref == capability.source_ref` and both trace to `lesson_section_grammar_categories.title`. The curated staging slug is NOT the content_unit identity.** Consequence: the `capability_content_unit_refs` bridge becomes **derivable by direct `source_ref` match** for grammar (instead of staging-supplied `contentUnitSlugs` metadata); and re-keying the existing grammar content_units is a deliberate one-time change (old curated-slug units are swept; the bridge re-points), NOT an inert re-seed.
...
> **Reconciled with ADR 0012 (2026-05-25):** translations are **Lesson-Stage** content. The canonical store is `lesson_section_item_rows.l1_translation` (NL) + `l2_translation` (EN), authored by the Lesson Stage. `learning_items.translation_nl/en` is a **derived dedup copy** the Capability Stage lifts when it reduces per-occurrence item rows into the global `learning_items` row — it does **not** generate translations (the EN enricher moves from `capability-stage/enrichEnTranslations.ts` to a lesson-stage enricher — migration plan §9.1). One canonical source (Rule #6): the lesson-side rows; the item-row column is the derived runtime read.
...
- Pipeline: the **Lesson Stage** authors the translation (on `lesson_section_item_rows`); the Capability Stage's `projectors/vocab.ts` *copies* the deduped value onto `learning_items.translation_*` — it no longer generates translations (ADR 0012).
...
| `lesson_sections` | survives | **Retain** `content` jsonb (Decision D amendment + §10.2). Add `section_kind` + `source_section_ref` **only**. The 8 display columns are **dropped** (ADR 0012 + Rule #10 — no reader; the blob serves display + coverage). |
...
| **NEW** `lesson_section_item_rows` | new | Decision D — vocab/expression/named-number items → `item` caps (capability-feeding) |
| **NEW** `lesson_section_grammar_categories` | new | Decision D — grammar categories → `pattern` caps (capability-feeding, typed; Capability Stage reads structure, no interpreter) |
| **NEW** `lesson_section_grammar_topics` | new | Decision D — grammar topic tags → `pattern` caps + overview chips |
| **NEW** `lesson_section_affixed_pairs` | new | ADR 0012 — morphology root→derived pairs → `affixed_form_pair` caps (DB form of `morphology-patterns.ts`) |
| ~~`lesson_section_pronunciation_letters`~~ | **DROPPED** | display-only (no capability consumer); rendered from `content.json` blob |
| ~~`lesson_section_exercise_groups`~~ | **DROPPED** | display-only (book's own exercises); rendered from `content.json` blob |
