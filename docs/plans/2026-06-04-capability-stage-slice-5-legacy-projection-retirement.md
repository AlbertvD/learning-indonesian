...
| D3 | `content_units` re-source | **Identity-only from the DB, stop writing `payload_json`.** Decision E retires the column in a separate migration. |
...
Replace `buildContentUnitsFromStaging` (`content-pipeline-output.ts:217`) with a DB-driven builder that emits the same `content_unit_key`/`unit_slug`/`source_ref`/`source_section_ref`/`unit_kind`/`display_order` from: `loaded.sections` (lesson_section units), `itemDbResult.items` (word/phrase learning_item units — **NOT** sentence/dialogue, which are being dropped), `patternDb.categories` (grammar_pattern units), and the affixed pairs (affixed_form_pair units). **`payload_json` is emitted empty/`{}`** (Decision E — unread; column drop deferred). Reuse the *exact* key/slug helpers (`contentUnitKey`, `stableSlug`, `sourceRefFor*`) from `content-pipeline-output.ts` so keys match the staging-derived ones.

**Step 1:** Failing test asserting identity fields match the staging builder's output for a fixture lesson, with `payload_json === {}`.
**Step 2–4:** Implement + green.
**Step 5:** Commit: `feat(capability): DB-driven content_units builder, identity-only (#147 5a)`.

> **Parity note:** the staging builder emits a learning_item unit per *deduped* `learningItems` entry including sentence/dialogue; the DB builder emits only word/phrase. The 70 sentence/dialogue units are intentionally absent — the parity gate (5a.6) treats them as the *expected* delta (they're deleted in 5b.10), NOT a parity failure. Encode this allowance explicitly in the parity assertion.
...
- Call the DB `content_units` builder; feed its output to `upsertContentUnits` **instead of** `staging.contentUnits` (but keep the staging regeneration running for now — its output is simply unused for the upsert). The capability_content_units junction (step 6) must be re-keyed off the DB content_units (match by `content_unit_key`/`unit_slug` → cap canonical keys); preserve the existing `relationshipKind` mapping.
...
A pure comparator asserting the DB-native residual output (audio caps, affixed caps, content_units identity) is set-equal to the staging-derived output for a fixture, **modulo the known sentence/dialogue content_unit delta** (allowlisted). This is the inert-ness proof.
- **Fixture MUST include at least one `sentence`/`dialogue_chunk` item** (data-arch N2) so the allowed-delta path is actually exercised — assert it's in `allowedDelta`, not a parity failure (else the gate passes vacuously).
