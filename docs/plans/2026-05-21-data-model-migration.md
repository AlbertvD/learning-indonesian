...
| 2 | **`item` source kind end-to-end** | Add `capability_audio_refs` table. Collapse `item_meanings` into `learning_items.translation_nl` + `translation_en` + `usage_note` columns; drop `item_meanings` (Decision R). Add 3 vocab-distractor tables: `recognition_mcq_distractors`, `cued_recall_distractors`, `cloze_mcq_item_distractors` (Decision G2 Group B) — wires up the orphan `vocab-enrichments.ts` agent output. Rewrite `byKind/item.ts` to (a) read translations from `learning_items.translation_<lang>` directly; (b) read curated distractors from the new tables when present, fall back to random pool when absent; (c) read upstream typed tables instead of `capability_artifacts`. Rewrite `session-builder/adapter.ts:282-289` planner-side artifact reader. Rewrite pipeline `projectors/vocab.ts` + audio path to write the new columns + new distractor tables + `capability_audio_refs`, stop emitting the 9 item-only artifact kinds, stop inserting `item_meanings` rows. Update `vocab-exercise-creator` agent prompt to write to the new tables. Republish all 9 lessons (republishing fills the new columns + distractor rows from staging). Verify item-source caps render with curated distractors (E2E asserts a known curated distractor appears, per the spec pattern in §13.5). Item-keyed `capability_artifacts` rows become stale (deleted in PR 7). | MEDIUM |
