...
decisions_resolved:    # 2026-05-31, operator + live-DB verification — these BIND; supersede the decisions_proposed defaults where they differ
  OQ2-1: "A — in-stage generation, confirmed (mirrors shipped Slice-1 OQ-1=A)."
  OQ2-4: "CUT grammar_pattern_examples. No reader anywhere (grep 0 in src/ + scripts/); byKind/pattern.ts reads only the 4 typed exercise tables; the lesson reader renders examples from each lesson's bundled content.json snapshot, not the DB. Examples are GENERATOR-INPUT ONLY, read from lesson_section_grammar_categories.examples — never persisted as a capability table. Update epic #100's acceptance criterion (drop 'write grammar_pattern_examples')."
  OQ2-2: "Option B — idempotency lives IN the stage, NO external table / sentinel column / transaction. (1) SEEDED-CHECK (load phase, loadFromDb): a pattern is seeded iff it has >=1 active row for EVERY exercise type its capabilities require. Complete -> skip generation. Partial or absent -> treat as unseeded -> delete the pattern's existing exercise rows across all 4 tables, then regenerate (delete-first-on-partial avoids duplicating the types that already landed). This SIDESTEPS the plan's 'seeded-but-partial' atomicity caveat because the signal is 'all required types exist', NOT 'any row exists'. (2) POST-WRITE CERTIFICATION (Capability Gate, new CS code): assert every pattern ended with full per-type coverage; fail loud otherwise (pattern_typed_row_missing class). (3) --regenerate <pattern-slug> = explicit destructive path (delete by grammar_pattern_id across the 4 tables + rebuild). Cross-lesson reuse = WARNING (Lesson #6)."
  OQ2-3: "As plan (Task 8). The exercise_review_comments FK is genuinely NEW schema work -> RE-DISPATCH a focused architect review before implementing Task 8; do not fold silently."
  OQ2-5: "PATTERN MODEL (resolved 2026-05-31, operator) — the slug source. The legacy grammar_patterns.slug values (e.g. 'bukan-negation') are LLM-authored, NON-reproducible, and NOT derivable from the typed grammar sections (titles slugify differently; 7 patterns vs 10 categories for L6; grammar_patterns.introduced_by_source_id NULL for all 47 — no DB link). The plan's 'byte-identical canonical keys' requirement was justified SOLELY by FSRS protection — and live DB proves that is moot: 0 of 745 capability_review_events and 0 of 476 learner_capability_state rows target ANY of the 94 pattern caps. Grammar was NEVER practiced. RESOLUTION: drop byte-identical; derive patterns DB-native from lesson_section_grammar_categories with ONE CATEGORY = ONE PATTERN: slug = stableSlug(category.title), explanation = category rules, examples = category.examples (generator input). canonical_key formula unchanged: cap:v1:pattern:lesson-{N}/pattern-{slug}:{pattern_recognition|pattern_contrast}:none:text:none. The 47 legacy grammar_patterns + 94 caps + their PR-4 typed exercise rows are REPLACED/retired (safe: 0 progress; use retireOrphanedCapabilities). Pattern set changes (L6: 10 vs legacy 7). CONTENT-QUALITY WATCH: some categories ('Duration','Day parts') are reference lists, not drill-worthy — the generator MAY decline to emit exercises for a category that yields none (WARN, not error). This SUPERSEDES Task 3's 'byte-identical' criterion and the legacy disk-authored slug provenance."
data_prerequisite: "Live DB 2026-05-31: grammar exists for L1-L9 (47 grammar_patterns rows), but the typed grammar sources the generator reads (lesson_section_grammar_categories) cover only L1,2,3,4,6,9,10 — L5/L7/L8 have ZERO typed grammar-category rows (PR 6 left them lint-blocked from re-publish; their examples still sit in capability_artifacts.pattern_example). Live trial uses L6 (has typed data). L5/7/8 grammar cutover is GATED on their re-publish — a separate prerequisite (same class as Slice 1's translation_nl scoping); document, do not block the build."
---
...
**[SUPERSEDED by OQ2-4 + the Implementation reconciliation — see that section.]** `grammar_pattern_examples` is CUT and is NOT written, so it is NOT the seeded signal. **The canonical "seeded?" signal is: a pattern is "seeded" iff it has ≥1 active row for EVERY required exercise type (count DISTINCT types, not rows) across the 4 typed exercise tables for its `grammar_pattern_id`.** Partial coverage → re-detect as unseeded → delete-first + full regenerate (no transaction needed — the signal is crash-safe by construction). `--regenerate <pattern-slug>` deletes the pattern's rows across all 4 typed tables then rebuilds. **Test the partial-failure path explicitly with a crash BETWEEN typed tables** (write tables 1–2, simulate crash before table 3, re-run → pattern re-detected unseeded → fully regenerated, no duplicates).
...
**Task 3 — Pure pattern projectors.** `grammar_patterns` upserts (by `slug`) + `pattern` capabilities (canonical keys), deterministically from the typed grammar sections (`lesson_section_grammar_categories`) — **one category = one pattern** (see the Implementation reconciliation for slug/NOT-NULL derivations). **NO `grammar_pattern_examples` write** (CUT, OQ2-4 — examples are generator-input only). Pure. **Cap shape reference:** `pattern_recognition` + `pattern_contrast`, `requiredArtifacts:[]`, `learnerLanguage:'none'`, sourceRef via `normalizeLessonSourceRef` (`capabilityCatalog.ts:119-156`). **NOT byte-identical to the legacy bundle** — the slug is `l{N}-{stableSlug(category.title)}` (NEW; legacy keys are superseded per OQ2-5, 0 pattern progress). canonical_key = `cap:v1:pattern:lesson-{N}/pattern-{slug}:{type}:none:text:none`. Assert within-lesson slug uniqueness (throw on unresolved collision). Commit.
...
**Task 9 — Retire `publish-grammar-candidates.ts`.** Confirm its writes are fully subsumed by the stage; delete it + its callers/Makefile target. **Account for `item_context_grammar_patterns`** (`publish-grammar-candidates.ts:244-255` upserts these grammar-pattern↔vocab-item-context junction rows; read only by `coverageService` Path A `:139-149`): decide explicitly — carry the junction write into the stage, or retire it with the legacy grammar surface (it cascades on `grammar_pattern_id`, so the Task 6 cutover-delete already clears stale rows). Coordinate with the Task 8 coverageService repoint. Commit.

**Task 10 — Integration + MANDATORY live trial (Lesson #1).** Canonical integration test (seed grammar fixtures → run → assert typed exercise + example rows → re-run idempotent → `--regenerate` replaces one pattern). **Then the live trial:** publish a real grammar-bearing lesson to the homelab — **use L6 (LIVE-VERIFIED 2026-05-31: 10 grammar categories, all 10 rule-bearing, 7/10 with examples — so 10 patterns, and the generator must tolerate the 3 rules-only categories)**; verify the 4 typed exercise tables written (no `exercise_variants` rows for the new lesson), NO `grammar_pattern_examples` write (CUT), `status=ok` + promotion, a grammar exercise renders from the typed tables, and a `capability_review_events` row lands. Sweep for CHECK-violation / answer-equal / cross-lesson-error classes (the 4 Slice-1 bug classes). Fix anything the trial surfaces.
