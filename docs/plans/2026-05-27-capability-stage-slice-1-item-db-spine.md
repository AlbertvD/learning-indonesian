...
> **RESULT (verified live 2026-05-31 after re-publishing lessons 1,2,3,4,6,9):** word/phrase items with null `translation_nl` = **0 / 619** — Slice 1's actual scope is **complete** (the column-restricted `upsertLearningItemIdempotent` refresh works). The "116" figure conflated item types: the remaining 116 nulls are **45 `dialogue_chunk` + 71 `sentence`** items — NEITHER is a word/phrase typed-path item. `dialogue_chunk` translation is the legacy/Slice-3 dialogue path; `sentence` items are exercise/grammar prompts (`"Pilih yang benar: …"`), a separate path. These are the anticipated "separate finding," out of Slice 1's scope. (Some also sit in L5/7/8, which are pre-existing-lint-blocked from re-publishing at all.)
