...
These exercise families must be implemented, published, and schedulable in the first rollout:
- `contrast_pair`
...
- existing live `recognition`, `cued_recall`, `typed_recall`, and `cloze` flows may continue using their current payload-building logic from canonical live tables
...
- only approved published content is scheduled
